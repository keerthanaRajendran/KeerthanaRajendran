        ...
        _scroll: function (e) {
            ...
            if (this._vScrollbar && this._scrollYdata && this.model) {
                if ((this._scrollYdata.scrollable - this.model.scrollOneStepBy) >= this.scrollTop()) {
                    if (!this.element.is(":hover")) proxy._off(ej.getScrollableParents(proxy.wrapper), "scroll", null);
                    window.onmousewheel = function (args) {
                        if (proxy.model && proxy.model.preventDefault && proxy.element.is(":hover")) args.preventDefault();
                    }
                }
            }
        },
