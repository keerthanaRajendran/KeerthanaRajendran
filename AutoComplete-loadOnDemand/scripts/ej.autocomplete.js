...
(function ($, ej, undefined) {
    ...
    ej.widget("ejAutocomplete", "ej.Autocomplete", {
       ...
       selectValueByKey: function (key) {
         ...
         if (this.model.multiSelectMode == "visualmode") {
             ...
             if (typeof currentValue == "number") currentValue = this._textFormateString(currentValue);
				...
				if(ej.isNullOrUndefined(this.value()))
				  this._modelValue =  currentValue + delimiterIndex ;
                else if (this.value().indexOf(currentValue) == -1 || !flag)
				   this._modelValue = this.value() + currentValue + delimiterIndex;
			    else
				   this._modelValue = currentValue + delimiterIndex;
        ...
        _destroy: function () {
            this.element.width("").removeAttr("role aria-label aria-expanded aria-haspopup aria-autocomplete autocomplete placeholder aria-owns aria-disabled disabled aria-activedescendant");
            ...
            this._hideResult();
            if(this.suggestionList)this.suggestionList.remove();
        },

        _initialize: function () {
            this.value(this.value() === "" ? this.element[0].value : this.value());
            this.element.attr("role", "combobox").attr("aria-label", "Autocomplete textbox").attr("aria-expanded", false).attr("aria-autocomplete", "list").attr("aria-activedescendant","null");
        ...
        _generateSuggestionList: function (e) {
            ...
            if (this._getLiTags().length > 0 && this._isFocused) {
				this._showResult(e);
			}
            ...
            if (_proxy.model.multiColumnSettings.enable) {
                ...
                for (var j = 0, listLength = list.length; listLength > j; j++) {
                    ...
                    if (!ej.isNullOrUndefined(_text)) {
						...
						li.attr({"role":"option","data-value":_text})
                        fragmentParent.appendChild(li[0]);
                        _proxy._currList = _proxy._currList.concat([list[j]]);
                    }
                }
        ...
        _addListHover: function () {
			...
			this.element.attr('aria-activedescendant',activeItem.attr("id"));
        ...
        _enterTextBoxValue: function (e) {
			if(this._timeStamp && -(this._timeStamp-e.timeStamp)<400) return;
        ...
        _RightToLeft: function (value) {
            if (value) {
                ...
                if(this.suggestionList)this.suggestionList.addClass("e-rtl");
            }
            else {
                this.wrapper.removeClass("e-rtl");
                 if(this.suggestionList)this.suggestionList.removeClass("e-rtl");
            }
			if(this.model.multiColumnSettings.enable && this.model.multiColumnSettings.showHeader && this._tableHeaderDiv) {
        ...
        _setRoundedCorner: function (value) {
            if (value) {
                ...
                if(this.suggestionList)this.suggestionList.addClass("e-corner");
            }
            else {
                this.container.removeClass("e-corner");
                if(this.suggestionList)this.suggestionList.removeClass("e-corner");
            }
        },
        ...
        _OnKeyDown: function (e) {
            ...
            switch (e.keyCode) {
                ...
                case 8:
					...
					if(this.element.hasClass("e-load") && (this._queryString && this._queryString.length <= this.model.minCharacter))
						e.preventDefault();
        ...
        _OnDropdownClick: function (e) {
            if (this.model.enabled && !this.model.readOnly) {
				...
				else if((this._queryString && this._queryString.length > 0) || (this.value()))
				{
				   if(ej.isNullOrUndefined(this._queryString))
					   this._queryString = this.value();
				   this._isFocused = true;
				   this._OnTextEnter(e);
				}
        ...
        _valueChange: function (e) {
            ...
            if (this.value() != currValue || (currValue == "" && this.element.val() != "")) {
				...
				this._changeEvtTrigger(currValue, e);				
			  if(this.model.multiColumnSettings.enable || this.model.multiSelectMode != "none")
				  this._queryString = null;
			   else
				  this._queryString = this.element.val();
            }
