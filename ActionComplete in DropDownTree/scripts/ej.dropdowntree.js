...
*/
(function (fn) {
    typeof define === 'function' && define.amd ? define(["./../common/ej.core","./../common/ej.globalize","./../common/ej.data","./../common/ej.scroller","./../common/ej.draggable","./../common/ej.treeview","./../common/ej.dropdownlist"], fn) : fn();
})
(function () {
    ...
    var ejDropDownTree = (function (_super) {
        ...
        ejDropDownTree.prototype.checkAll = function () {
            if (this.model.treeViewSettings.showCheckbox) {
                ...
                if (eles.length != this.getLi().length)
        ...
        ejDropDownTree.prototype.finalize = function () {
            if ((!ej.isNullOrUndefined(this.value()) && this.value() === '') || this.value() !== this.element.val()) {
                if (this.treeView.model.allowMultiSelection || this.treeView.model.showCheckbox)
        ...
        ejDropDownTree.prototype.setValues = function (value) {
            this._setValuesCalled = true;
            this.treeNodesSelection([]);
            this.setValueText(value, 'value');
            this._setValuesCalled = false;
        };
                ...
                if (!(this.treeView.dataSource() instanceof ej.DataManager)) {
                    ...
                    for (var i = 0; i < treeSrc.length; i++) {
                        if (!ej.isNullOrUndefined(val) && typeof (val) == "string" || typeof (val) == "number") {
                            ...
                            if (!this.treeView.model.allowMultiSelection && !this.treeView.model.showCheckbox) {
                                val = val[0];
                                this.setValueText(val, fieldType);
                            }
                            for (var k = 0; k < val.length; k++) {
                                if (ej.getObject(mappedField[0], treeSrc[i]) === val[k]) {
                    ...
                    for (var i = 0; i < treeSrc.length; i++) {
                        ...
                        if (typeof (val) == "string" || val == null) {
                            ...
                            if (!ej.isNullOrUndefined(treeSrc[i].child)) {
                                for (var j = 0; j < treeSrc[i].child.length; j++) {
                                    nodeIndex = nodeIndex + 1;
                                    if (ej.getObject(mappedField[1], treeSrc[i].child[j]) === val) {
                                        this.treeNodeSelection(nodeIndex);
                                    }
                                }
                            }
                            ...
                            for (var m = 0; m < val.length; m++) {
                                if (ej.getObject(mappedField[0], treeSrc[i]) === val[m]) {
                                    selectedNodes.push(nodeIndex);
                                }
                                if (!ej.isNullOrUndefined(treeSrc[i].child)) {
                                    for (var j = 0; j < treeSrc[i].child.length; j++) {
                                        nodeIndex = nodeIndex + 1;
                                        if (ej.getObject(mappedField[1], treeSrc[i].child[j]) === val[m]) {
                                            selectedNodes.push(nodeIndex);
                                        }
                                        if (j == treeSrc[i].child.length - 1 && m != val.length - 1) {
                                            nodeIndex = nodeIndex - treeSrc[i].child.length;
                                        }
                                    }
                                }
                            }
        ...
        ejDropDownTree.prototype.treeNodesSelection = function (index) {
            if (!ej.isNullOrUndefined(this.treeView)) {
                if (this.model.treeViewSettings.showCheckbox) {
                    this.treeView.setModel({ checkedNodes: index });
                }
                else {
                    this.treeView.setModel({ selectedNodes: index });
                }
            }
        };
        ...
        ejDropDownTree.prototype.treeMethods = function () {
            ...
            ej.TreeView.prototype.checkAll = function () {
                checkAll.call(this);
                if (this.element.parents()[2].classList.contains("e-ddtree-popup"))
                    proxy.checkAll();
            };
            ...
            ej.TreeView.prototype.unCheckAll = function () {
                unCheckAll.call(this);
                if (this.element.parents()[2].classList.contains("e-ddtree-popup"))
                    proxy.unCheckAll();
            };
            ej.TreeView.prototype.uncheckNode = function (ele) {
                uncheckNode.apply(this, [ele]);
        ...
        ejDropDownTree.prototype.checkElements = function (args) {
            ...
            for (var i = 0; i < this.valueContainer.length; i++) {
                ...
                for (var j = 0; j < eles.length; j++) {
                    ...
                    if (!(ej.isNullOrUndefined(this.model.treeViewSettings.fields))) {
                        if (this.getMappedField(this.valueContainer[i], "value", "text") === eleText) {
                            flag = true;
                            break;
                        }
                        else if (!(this.treeView.dataSource() instanceof ej.DataManager)) {
                            flag = false;
                        }
                    }
                    else {
                        if (this.valueContainer.indexOf(eleText) > -1) {
                            flag = true;
                            break;
                        }
                        else {
        ...
        ejDropDownTree.prototype.ensureRootCheck = function (args) {
            ...
            for (var i = 0; i < rootEle.length; i++) {
                if ($(rootEle[i]).parent('li').length) {
                    ...
                    if (!(ej.isNullOrUndefined(this.model.treeViewSettings.fields)))
                        var value = (!(ej.isNullOrUndefined(this.model.treeViewSettings.fields.text) && ej.isNullOrUndefined(this.model.treeViewSettings.fields.value))) ? this.getMappedField(textContent, "text", "value") : textContent;
                    else
                        var value = textContent;
                    if (this.checkContains(value)) {
        ...
        ejDropDownTree.prototype.getMappedField = function (text, textfield, valuefield) {
            ...
            if (!(this.treeView.dataSource() instanceof ej.DataManager)) {
                ...
                for (var i_1 = 0; i_1 < treeSrc.length; i_1++) {
                    if (ej.getObject(mappedtextField[0], treeSrc[i_1]) == text) {
                        val = ej.getObject(mappedvalueField[0], treeSrc[i_1]);
                    }
                }
            }
            else if ((this.treeView.dataSource() instanceof ej.DataManager) && this.isSingleSelect()) {
                for (var i = 0; i < this.getSelectedElements().length; i++) {
                    val = this.getSelectedElements()[i].id;
                }
            }
            return val;
        };
