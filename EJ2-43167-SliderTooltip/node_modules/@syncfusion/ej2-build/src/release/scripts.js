...
gulp.task('global-scripts', ['blazor-compile'], async function () {
    ...
    if (config.blazorDependencies && config.blazorDependencies.ignoreScripts) {
        return;
    }
    ...
    if (fs.existsSync('./third-party/config.json') && config.blazorDependencies && !config.blazorDependencies.isSingleBundle) {
        ...
        if (commonDeps.length > 0) {
            ...
            await bundle.write({
                ...
                format: 'iife',
                name: 'sf.curInst',
                footer: `${blazorDeps.commonClassName ? `sfBlazor.modules["${packName}base"] = "${packName}.${blazorDeps.commonClassName}";` : ''}
window.sf.${packName} = window.sf.base.extend({}, window.sf.${packName}, ${packName}base({}));`
            });
        ...
        if (configJson && configJson.components && configJson.components.length > 0) {
            ...
            for (var comp of comps) {
                ...
                await bundle.write({
                    ...
                    format: 'iife',
                    name: 'sf.curInst',
                    footer: `sfBlazor.modules["${moduleName}"] = "${packName}.${compConfig.compClassNames[comp]}";`
                });
                ...
                if (matchReg && matchReg[1]) {
                    ...
                    var depJson = fs.existsSync('./third-party/') ? `sfBlazor.dependencyJson.${moduleName}` :
                        JSON.stringify(dependencies.replace(/sf\./g, '').replace(')', '').split(','));
                    var blazorAdaptor = currentPackage == 'data' ? `
sfBlazor.initBlazorAdaptor();` : '';
                    var replaceContent = `sfBlazor.loadDependencies(${depJson}, () => {
    ...
    // blazor library or single bundle scripts
        ...
        if (matchReg && matchReg[1]) {
            var dependencies = matchReg[1].replace(/ej2/g, 'sf.').toLowerCase();
            var depJson = (fs.existsSync('./third-party/') || config.blazorDependencies) ? `sfBlazor.dependencyJson.${currentPackage}` :
                JSON.stringify(dependencies.replace(/sf\./g, '').replace(')', '').split(','));
            var blazorAdaptor = currentPackage == 'data' ? `
        sfBlazor.initBlazorAdaptor();` : '';
            var baseScript = currentPackage !== 'base' ? `sfBlazor.libs.push("${currentPackage}")` : '';
            var libScript = config.blazorDependencies && config.blazorDependencies.isSingleBundle ? getSingleBundleModules(packName) :
                baseScript;
            var replaceContent = `});
${libScript}
sfBlazor.loadDependencies(${depJson}, () => {
    sf.${currentPackage} = sf.${currentPackage}({});${blazorAdaptor}
            ...
            blazorContent += currentPackage !== 'base' ? `\nsfBlazor.libs.push("${currentPackage}");` : '';
        }
