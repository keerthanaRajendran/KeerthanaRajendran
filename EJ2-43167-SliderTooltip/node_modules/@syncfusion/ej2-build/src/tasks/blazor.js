...
gulp.task('blazor-scripts', function () {
    try {
        ...
        var blazorPath = './third-party/blazor';
        var blazorScriptPath = `${blazorPath}/Scripts`
        var blazorDepsFile = `${blazorPath}/build/dependencies.json`;
        var currentPackage = common.currentPackage.replace('ej2', '').replace(/-/g, '');

        // generate dependency json for blazor dynamic script import
        var config = common.config();
        var blazorDeps = config.blazorDependencies || {};
        var comps = Object.keys(blazorDeps);
        var compAllDeps = {}

        var finalJson = {};
        if (fs.existsSync(blazorDepsFile)) {
            finalJson = JSON.parse(fs.readFileSync(blazorDepsFile, 'utf8'));
            // delete finalJson[currentPackage];
        }
        finalJson["singleBundles"] = finalJson.singleBundles ? finalJson.singleBundles : {};
        if (blazorDeps.isSingleBundle) {
            finalJson.singleBundles[currentPackage] = [];
        }
        for (var comp of comps) {
            if (comp !== 'common' && comp !== 'commonClassName' && comp !== 'isSingleBundle') {
                var blazorCompDeps = blazorDeps[comp];
                var compDeps = [];
                var internalCompDeps = (blazorCompDeps.internal && blazorCompDeps.internal.length > 0) ? blazorCompDeps.internal : [];
                var internalDepComps = [];
                for (var internalCompDep of internalCompDeps) {
                    var depCompName = internalCompDep.match(/[^\/]+/)[0].replace(/-/g, '');
                    if (internalDepComps.indexOf(depCompName) === -1) {
                        internalDepComps.push(depCompName);
                    }
                }
                var compAllDeps = compDeps.concat(internalDepComps);
                if (blazorCompDeps.external && blazorCompDeps.external.length > 0) {
                    compAllDeps = compAllDeps.concat(blazorCompDeps.external.map((val) => { return val.replace(/-/g, ''); }));
                }
                finalJson[comp.replace(/-/g, '')] = compAllDeps;
                if (blazorDeps.isSingleBundle) {
                    finalJson.singleBundles[currentPackage].push(comp.replace(/-/g, ''));
                }
            }
        }

        if (config.blazorNamespace) {
            var scripts = Object.keys(config.blazorNamespace).map(value => config.blazorNamespace[value]);
            scripts.forEach(script => {
                var moduleName = script.replace('.js', '');
                finalJson[moduleName] = [moduleName];
            });
        }

        fs.writeFileSync(blazorDepsFile, JSON.stringify(finalJson, null, '\t'));
        ...
        // fs.writeFileSync(`${blazorScriptPath}/modules/${currentPackage}.js`, scriptContent);
    }
    catch (e) {
        console.log(e);
    }
